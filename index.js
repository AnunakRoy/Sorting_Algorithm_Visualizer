...
slider.addEventListener("input", function () {
  ...
  renderBars(unsorted_array); //Shows the bar animation
});

speed.addEventListener("change", (e) => {
  speedFactor = parseInt(e.target.value); //e.target.value retrieves the value of the selected option from the dropdown menu
  //This value is then converted to an integer using parseInt(), and the result is assigned to the speedFactor variable.
});

let algotouse = "";

select_algo.addEventListener("change", function () {
  algotouse = select_algo.value;// The value of the selected option from the dropdown menu is obtained using select_algo.value. 
  //This value is then assigned to the algotouse variable.
});

function randomNum(min, max) {// Generates a random number between minRange and maxRange(used in createRandomArray() function)
...
function renderBars(array) {
  for (let i = 0; i < numOfBars; i++) {
    let bar = document.createElement("div"); //document.createElement() method creates the HTML element specified by tagName
    bar.classList.add("bar"); //"bar" added to css
    //bar.style.top = "0";
...
randomize_array.addEventListener("click", function () {
  unsorted_array = createRandomArray();
  bars_container.innerHTML = ""; //Clears all prior bars
  renderBars(unsorted_array);
});
    ...
    for (let j = 0; j < array.length - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        for (let k = 0; k < bars.length; k++) {
          if (k !== j && k !== j + 1) {
            bars[k].style.backgroundColor = "#C38154";
          }
        }
        let temp = array[j];
...
async function partition(items, left, right) { //Hoare Partition for Quick Sort
  let bars = document.getElementsByClassName("bar");
  let pivotIndex = Math.floor((right + left) / 2);
  var pivot = items[pivotIndex]; //middle element
  bars[pivotIndex].style.backgroundColor = "#2E8A99";

  for (let i = 0; i < bars.length; i++) {
    if (i != pivotIndex) {
      bars[i].style.backgroundColor = "#C38154";
    }
  }
